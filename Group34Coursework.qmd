...
Initially, a Poisson generalized additive model with a log link was considered for count data of TB cases by microregion. However, standardizing the count by population was necessary, and using the log of population in the model was found to be the most effective method. To achieve this, an offset for the log of population was included in the model in R, resulting in the tuberculosis rate per capita.

Let the model be : <br> $$ Log(\lambda_i) = offset(log(Population_i)) + f_1(Indigenous_i) + $$ <br> $$ f_2(Illiteracy_i) + \\f_3(Urbanisation_i) + f_4(Density_i) + f_5(Poverty_i) + $$ <br> $$ f_6(PoorSanitation_i) +\\ f_7(Unemployment_i) + f_8(Timeliness_i) + $$ <br> $$ f_9(Year_i) + f_{10}(Year_i, lon_i) + \\ f_{11}(lat_i,Year_i), $$ <br> $$ {u_i} = E[TB_i] ~~ and  ~~ TB_i \sim Pois(\lambda_i) $$
...
Next, we needed to check if our model was accurate. Unfortunately, we found that the distribution of our data didn't look like a theoretical Poisson distribution, as shown by a QQ-plot.

To check if this was because of over-dispersion, we calculated the Pearson estimate of our dispersion parameter, which was almost 7 when it should have been 1. This means that our data was over-dispersed for a Poisson model.

The Negative Binomial (NB) model is an alternative to the Poisson model and is used for count data. It can adjust the variance independently from the mean, which makes it more flexible than the Poisson model. The NB model was used on the data with the same specification as before, and it was fortunate that the revised model showed a good fit. 

#TODO ADD NEGATIVE BINOMIAL LATEX

Let the model be : <br> $$\log(\mu_i) = \beta_0 + f_1(x_{i1}) + f_2(x_{i2}) + \ldots + f_k(x_{ik}) + \log(\phi) + \log(y_i+r_i)$$

The estimate for the dispersion parameter was 1.133 and the AIC was reduced. The final model was achieved after adjusting the choice of rank for each of the smooth terms until they were not too low.
