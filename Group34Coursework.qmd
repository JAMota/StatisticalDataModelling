...
```
The results are terrible. Our qq-plot suggest that the quantiles in our data our not very similar to the line as it deviates from the line in nearly all the values.
As such this suggests that our current model doesn't fit the data correctly and required another distribution.

## TODO better prepare the residuals and explain them if we have enough figures to show


We will now use the Pearson estimate for the dispersion parameter, this method allow us to estimate the amount of extra variability, or over-dispersion in count data and therefore analyse if the Poisson distribution assumption of equal mean and variance holds.
